...
meshopt_Bounds meshopt_computeClusterBounds(const unsigned int* indices, size_t index_count, const float* vertex_positions, size_t vertex_count, size_t vertex_positions_stride)
{
	...
	//SpeedEngine: MeshOptimizer:HierarchicalMeshletBounds:[yunhsiaowu]:[BEGIN]
	// Support more triangles than hardcoded kMeshletMaxTriangles
	meshopt_Allocator allocator;
	float(*normals)[3] = allocator.allocate<float[3]>(index_count / 3);
	float(*corners)[3][3] = allocator.allocate<float[3][3]>(index_count / 3);
	//SpeedEngine: MeshOptimizer:HierarchicalMeshletBounds:[yunhsiaowu]:[END]
	size_t triangles = 0;

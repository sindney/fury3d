...
namespace fury
{
	namespace Editor
	{
		...
		// requested `type` (Mesh / Material / Texture) registered in the
		// active Scene's EntityManager as a Selectable row. Single-click
		// selects (highlights); double-click or `OK` confirms and invokes
		// `onPick` with the chosen asset (as shared_ptr<void>), then
		// closes the popup. `Cancel` closes without invoking. Mirrors
		// the RenderOpenImportModal pattern (Editor.cpp:373-438).
		//
		...
		// / texture) can be open simultaneously without aliasing.

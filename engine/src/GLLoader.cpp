...
static void LoadExtByName(const char *extensionName)
{
	...
	if(entry)
	{
		if(entry->LoadExtension)
		{
			...
			{
				*(entry->extensionVariable) = 1 + numFailed;
			}
		}
		else
		{
...
int gl::IsVersionGEQ(int majorVersion, int minorVersion)
{
	if(g_major_version == 0)
		GetGLVersion();

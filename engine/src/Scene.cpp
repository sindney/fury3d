...
namespace fury
{
	...
	Scene::Ptr Scene::Create(const std::string &name, const std::shared_ptr<SceneManager> &sceneManager)
	{
		return std::make_shared<Scene>(name, sceneManager);
	}

	Scene::Scene(const std::string &name, const std::shared_ptr<SceneManager> &sceneManager) :
		Entity(name)
	{

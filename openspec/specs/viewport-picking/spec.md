...
GPU-accelerated offscreen picking for the editor viewport: an R32UI ID-texture framebuffer that resolves left-clicks in the central viewport area to the front-most SceneNode, updating the shared editor selection within two frames. Drives the Scene Inspector highlight, the Node Properties panel, and the TRS gizmo anchor.

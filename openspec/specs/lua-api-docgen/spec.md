...
Automatic generation of the `docs/LUA_API.md` reference from the C++ bindings in `engine/Fury/LuaBindings.cpp`. Covers the docgen tool itself, its CMake integration as a `POST_BUILD` step of the `fury` target, and the resulting split of `docs/LUA.md` (narrative) vs `docs/LUA_API.md` (canonical per-binding reference).

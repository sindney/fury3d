...
Defines the in-memory texture model that lets importers (notably the glTF importer reached via FBX2glTF) upload embedded image bytes to the GPU directly from memory and lets the save path extract those bytes to sibling files when a scene is serialized to disk. This eliminates the temp-file extraction hop that previously left saved scenes referencing absolute temp paths and produced different results between the CLI write path and the runtime read path.

...
Editor-side detection and correction of unit-mismatched glTF/FBX imports. The engine's unit convention is 1 unit = 1 cm; assets authored in metres (common for glTF samples and many DCC exports) arrive ~100× undersized. This capability prompts the user with a bounds-derived scale factor instead of silently rendering tiny scenes.

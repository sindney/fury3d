...
Per-platform DPI awareness for the editor. Declares per-monitor V2 DPI awareness on Windows via an embedded manifest, reads the system DPI at `Engine::Initialize` via `GetSystemDPI()` (with platform-specific implementations), and applies the result to the editor's GUI scale via two new `EngineOptions` fields and a `0.0f` sentinel. Migrates the ImGui font scale to `style.FontScaleMain` (ImGui 1.92) with a sentinel that follows the resolved `gui_scale` so fonts and widget layout track together on HiDPI displays.

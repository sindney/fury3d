...
altitude, scale, wind, density, fade distance), and the atmosphere
coefficients, plus the typed `SceneNode` accessors
(`AddComponent`/`GetComponent`/`RemoveComponent` overloads and a
`GetSkyAtmosphere()` getter per the existing per-type pattern).

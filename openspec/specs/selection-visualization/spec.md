...
Editor-only wireframe overlay drawn for the currently selected `SceneNode` inside the Viewport window. Reuses the engine's existing `RenderUtil` debug-draw primitives (`DrawBoxBounds` / `DrawMesh`) so the selection feedback visually matches the Profiler's debug overlays (point-light sphere, spot-light cone, mesh AABB), with a distinct editor-defined color so the two systems remain visually distinguishable when both are enabled. Driven by a `Signal<SceneNode*>` selection-changed event so future consumers (e.g. transform inspectors, property panels) can react without polling.

...
The `furye-cli render-mesh` CLI subcommand for visual debugging of the mesh-rendering
...
The subcommand moved from `fury` to `furye-cli`: it is compiled into the `furye-cli` launcher branch only (the player binary's main path no longer routes it).

#### Scenario: Render a mesh to a PNG

- **WHEN** the user runs `furye-cli render-mesh Resource/Scene/scene.json T90 /tmp/t90.png`
...
- **WHEN** the user runs `furye-cli render-mesh scene.json NonExistent /tmp/out.png`
...
- **THEN** the developer can run `furye-cli render-mesh scene.json <mesh> /tmp/out.png` to see the new look as a static image

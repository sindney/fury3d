...
- **File**:
  ...
  - `Package…` — no shortcut. Enabled only when a scene with a saved on-disk path is loaded; opens the package settings modal driving `furye-cli` (see the `Package...` flow requirement below).
  - (Separator)

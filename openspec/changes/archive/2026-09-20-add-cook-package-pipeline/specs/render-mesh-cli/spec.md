...
The subcommand moves from `fury` to `furye-cli`: it is compiled into the `furye-cli` launcher branch only (the player binary's main path no longer routes it).

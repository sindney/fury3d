...
This is the single canonical save-dispatch entry point used by the CLI (`convert scene`, `verify --round-trip`), the editor (`Editor.lua`'s `write_scene_to_path`), and any future caller that wants format-by-extension behavior.
...
This invariant is what `fury verify --round-trip` checks; it is the contract that makes the engine's save format usable as a session persistence mechanism.

...
The `fury render-mesh <scene> <mesh_name> <output.png>` CLI subcommand loads a scene, finds the named mesh, renders it to a 256×256 offscreen FBO using the same simple Lambert shader the editor's mesh preview / thumbnail use, and writes a PNG file. The subcommand exists to let developers iterate on the mesh-rendering pipeline (shader, camera math, FBO setup) without launching the editor.

...
Per-asset editor windows opened from the Content Browser. Provides modal editor windows for `Mesh` and `Material` assets with typed widget editors (Material) and a 3D preview pane with orbit/zoom (Mesh).
...
### Requirement: Double-clicking a Mesh, Material, or ParticleSystem asset in the Content Browser SHALL open a per-asset editor window keyed on the asset's path

The Content Browser SHALL open a per-asset editor window when the user double-clicks a `Mesh`, `Material`, or `ParticleSystem` tile. The window SHALL be scoped to that single asset with title `<AssetType>: <asset basename>` (the basename is the file path's last segment, e.g. `Mesh: Cube` for path `content/Meshes/Cube.asset`). Only one editor window per asset SHALL be open at a time; double-clicking an already-open asset MUST focus the existing window rather than opening a duplicate. Closing the window (Ok / Esc / click-outside) SHALL NOT mutate the asset beyond edits already applied through the editor's widgets — the editor is a live view of the asset.

The "already-open" lookup SHALL key on the asset's file path (the canonical identity under `asset-path-identity`), not on its display name. Two assets at different paths that happen to share a basename (e.g. `content/A/cube.asset` and `content/B/cube.asset`) open independent editor windows — they are distinct assets even though their title strings collide. The ImGui window ID SHALL embed the path, not the name, so back-to-back renaming does not lose the editor's tracked window.

#### Scenario: Double-click opens the asset editor

- **WHEN** the user double-clicks a Mesh tile whose path is `content/Meshes/Cube.asset` in the Content Browser
...
- **WHEN** the editor window for `content/Meshes/Cube.asset` is already open
- **AND** the user double-clicks the same tile again
- **THEN** no second window is opened
- **AND** the existing window for that path is brought to the front

#### Scenario: Two assets sharing a basename get independent editor windows

- **WHEN** `content/A/cube.asset` and `content/B/cube.asset` are both in the EM
- **AND** the user opens an editor for `content/A/cube.asset`
- **AND** then opens an editor for `content/B/cube.asset`
- **THEN** two independent editor windows exist
- **AND** each window's title is `Mesh: cube.asset`
- **AND** editing one asset does not affect the other
...
- **WHEN** the user double-clicks a `ParticleSystem` tile whose path is `content/Particles/FireEmber.asset`
...
The `popup_id` SHALL be derived from the asset's path (under `asset-path-identity`), so two assets at different paths that share a display basename get independent orbit cameras.
...
- **WHEN** the user opens a Mesh editor for `content/Meshes/Cube.asset` and a Particle editor for
  `content/Particles/FireEmber.asset` side by side
...
- **WHEN** the user clicks "Browse…" on the diffuse_texture row and picks a texture from the texture picker
- **THEN** the row's thumbnail updates to the picked texture immediately
...
- **WHEN** the user opens the Mesh editor for a mesh whose path is `content/Meshes/Cube.asset`
...
- **AND** the orbit state for that mesh's `popup_id` (derived from the path) is retained in the `g_OrbitState` map until the mesh is evicted from the scene (so reopening the editor restores the previous camera angle)
...
- **Rotate**: left-mouse-drag orbits the camera around the mesh's AABB center (azimuth + elevation), using the existing `OrbitState` map keyed on `popup_id` (path-derived under `asset-path-identity`).
...
The viewer pane SHOULD fill the left/main region of the Mesh editor window (per the two-pane layout requirement added by this change).
...
The Mesh editor MUST render a metadata block with the mesh's serializable properties (rendered in the right-side metadata pane per the two-pane layout requirement added by this change):

- Name (read-only label; shows the basename)
